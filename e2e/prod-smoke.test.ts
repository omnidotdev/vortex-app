...
// ─── Landing page accessible when authenticated ─────────────────────────────
// Authenticated users should see the landing page at `/` with a
// "Go to Dashboard" CTA instead of "Get Started Free".

test.describe("landing page accessibility when authenticated", () => {
  test("authenticated user should see landing page at root URL", async ({
    page,
  }) => {
    await page.goto("/");
    await page.waitForLoadState("networkidle");

    // Should stay on the landing page, not redirect to /workspaces
  ...
  test("authenticated user should see dashboard CTA instead of sign-in", async ({
    page,
  }) => {
    await page.goto("/", { waitUntil: "networkidle" });

    // Should show "Go to Dashboard" link instead of "Get Started Free"
    await expect(
...
// ─── Aether (billing) health check ──────────────────────────────────────────

baseTest.describe("Aether health check", () => {
  baseTest(
    "billing service should resolve and respond",
    async ({ request }) => {
      const response = await request.get("https://billing.omni.dev/health", {
        timeout: 15_000,
      });

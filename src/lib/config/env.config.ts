...
// Internal billing URL for server-to-server communication
// Falls back to BILLING_BASE_URL for non-Docker environments
export const BILLING_INTERNAL_URL =
  typeof window === "undefined"
    ? (serverEnv.BILLING_INTERNAL_URL as string | undefined) || BILLING_BASE_URL
    : BILLING_BASE_URL;

export const API_GRAPHQL_URL = API_BASE_URL
  ? `${API_BASE_URL}/graphql`
  : undefined;

// Internal GraphQL URL for server-side requests
export const API_INTERNAL_GRAPHQL_URL = API_INTERNAL_URL
  ? `${API_INTERNAL_URL}/graphql`
  : undefined;
...
// Startup warnings for optional integrations (auth, billing, flags)
if (typeof window === "undefined") {
  if (!AUTH_CLIENT_ID)
    console.warn("AUTH_CLIENT_ID not set, managed auth disabled");
  if (!AUTH_CLIENT_SECRET)
    console.warn("AUTH_CLIENT_SECRET not set, managed auth disabled");
  if (!process.env.AUTH_SECRET)
    console.warn("AUTH_SECRET not set, managed auth disabled");
  if (!SERVER_AUTH_BASE_URL)
    console.warn("VITE_AUTH_BASE_URL not set, managed auth disabled");
}
if (typeof window === "undefined") {
  if (!BILLING_BASE_URL)

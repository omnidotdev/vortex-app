...
/**
 * Defer a value to the client to prevent hydration mismatch when the
 * server cannot know the value (e.g. session-derived current user ID)
 */
function useClientValue<T>(value: T): T | undefined {
...
function MemberRow({
  ...
  const deferredUserId = useClientValue(currentUserId);
  const isCurrentUser = member.userId === deferredUserId;

...
/**
 * ErrorBoundary that catches render errors inside the ReactFlow canvas,
 * preventing them from tearing down the entire page.
 */
class WorkflowEditorErrorBoundary extends React.Component<
      ...
      return (
        <div className="flex h-full flex-col items-center justify-center gap-4 p-8">
          <h2 className="font-semibold text-lg">
            Workflow editor encountered an error
          </h2>
...
function WorkflowEditorPage() {
  ...
  // Per-instance counter for generating unique node IDs (avoids SSR leak)
  const nodeIdCounterRef = useRef(0);
  const getNodeId = useCallback(
    () => `node_${Date.now()}_${nodeIdCounterRef.current++}`,
    [],
  );
  ...
  // Wrapped in try/catch so corrupted definitions degrade to an empty canvas
  let initialNodes: Node[];
  let initialEdges: Edge[];
  try {
    const rawDefinition =
      (workflow.definition as Record<string, unknown>) || {};
    const { nodes: parsedNodes, edges: parsedEdges } = isDslFormat(
      rawDefinition,
    )
      ? dslToReactFlow(
    ...
    initialNodes = ensureStepNames(parsedNodes);
  ...
  const executeConnectedActions = useCallback(
    async (triggerId: string, visited: Set<string> = new Set()) => {
      // Cycle detection: skip already-visited nodes to prevent infinite recursion
      if (visited.has(triggerId)) return;
      ...
      for (const edge of connectedEdges) {
        ...
        if (targetNode) {
          ...
          // Recursively execute connected actions (pass visited set for cycle detection)
          await executeConnectedActions(targetNode.id, visited);
        }
      }
    },
    [logToDebugPane],
